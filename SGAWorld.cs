...
namespace SGAmod
{
    public class SGAWorld : ModWorld
    {
        ...
        public override void PostWorldGen()
        {
            ...
            for (int i = 1; i < 3; i++)
            {
                int[] itemsToPlaceInOvergrownChestsSecond = new int[] {mod.ItemType("DecayedMoss"), mod.ItemType("DecayedMoss"), mod.ItemType("DecayedMoss"), mod.ItemType("DecayedMoss"), mod.ItemType("Biomass"), mod.ItemType("Biomass") };
                int itemsToPlaceInOvergrownChestsChoiceSecond = 0;
                for (int chestIndex = 0; chestIndex < 1000; chestIndex++)
                {
                    ...
                    if (chest != null && (WorldGen.genRand.Next(0,100)<10 || Main.tile[chest.x, chest.y-1].wall == mod.TileType("SwampWall")))
                    {
                        ...
                        for (int inventoryIndex = 0; inventoryIndex < 40; inventoryIndex++)
                        {
                            if (chest.item[inventoryIndex].type == 0 && WorldGen.genRand.Next(0, 100) < 10)
                            {
                                itemsToPlaceInOvergrownChestsChoiceSecond = Main.rand.Next(itemsToPlaceInOvergrownChestsSecond.Length);
                                ...
                                chest.item[inventoryIndex].stack = Main.rand.Next(1, Main.rand.Next(1, 3));
                                break;
                            }
                        }
