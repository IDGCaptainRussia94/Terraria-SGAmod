	...
	public class SeraphimShardProjectile : ModProjectile,IDrawAdditive
	{
		...
		private void DoAttack(NPC enemy)
        {
			...
			if (projectile.ai[0] > 0)
			{
				...
				if (projectile.ai[0] > attackperiod-30)
				{
					projectile.localAI[1] += 2.50f;
				}
			}
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			if (projectile.localAI[1] > 0)
			{
				...
				{
					spriteBatch.Draw(tex2, projectile.oldPos[i] - Main.screenPosition, null, prismColor * MathHelper.Clamp(projectile.localAI[1] / 75f, 0f, 1f) * (1f-(i / (float)projectile.oldPos.Length)), velAdd+projectile.rotation + MathHelper.Pi, tex2.Size() / 2f, projectile.scale, default, 0);
				}
			}
