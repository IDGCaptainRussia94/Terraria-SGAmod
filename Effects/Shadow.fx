...
float GetLumaInvert(float3 colors)
{
...
float4 ColorFilterInvertedFunction(float2 coords : TEXCOORD0) : COLOR0
{
    float4 voxelOverlayTexture = tex2D(overlaytexsampler, coords);
    ...
    float luminosity = GetLumaInvert(voxelOverlayTexture.r);//(voxelOverlayTexture.r + voxelOverlayTexture.g + voxelOverlayTexture.b) / 3;
        ...
        if (GetLumaInvert(tex1).r>edgeSmooth)
        return colorOutline;
                if (GetLumaInvert(tex2).r>edgeSmooth)
        return colorOutline;
                if (GetLumaInvert(tex3).r>edgeSmooth)
        return colorOutline;
                if (GetLumaInvert(tex4).r>edgeSmooth)
        return colorOutline;
...
technique Technique1
{
        pass ColorFilter
    {
        ...
        pass ColorFilterInverted
    {
        PixelShader = compile ps_2_0 ColorFilterInvertedFunction();
    }
    }