...
namespace SGAmod.Items.Weapons
{
	public class QuasarCannon : SeriousSamWeapon
	{
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			if (player.ownedProjectileCounts[mod.ProjectileType("QuasarCannonCharging")]<1){
			int proj=Projectile.NewProjectile(position.X, position.Y, speedX, speedY, mod.ProjectileType("QuasarCannonCharging"), damage, knockBack, player.whoAmI);
		}
			return false;
		}
	...
	public override bool? CanHitNPC(NPC target){return false;}
		...
		player.itemAnimation=6;
			...
			if (Main.netMode == NetmodeID.Server || Main.netMode==NetmodeID.SinglePlayer)
			{
				...
				direction.Normalize();
				projectile.Center = player.Center + direction * 72;
